...
public class ReplyBbsDAOImp implements ReplyBbsDAO {
  ...
  public ReplyBbs findById(Long rid) {
    ...
    sql.append("select rid, bid, content, username, created_at, updated_at ");
  ...
  public Long addReply(ReplyBbs replybbs, Long bid) {
    ...
    // 댓글을 저장할 sql 쿼리문 작성
    sql.append("insert into replybbs(rid, bid, content, username, created_at , updated_at)  ");
    sql.append("    values(replybbs_rid_seq.nextval , :bid , :content , :userName , sysdate ,sysdate)  ");
  ...
  public Long UpdateReply(Long rid ,ReplyBbs replyBbs) {
    ...
    sql.append("set content = :content , username = :userName , updated_at = sysdate ");
    sql.append("        where rid = :rid ");
