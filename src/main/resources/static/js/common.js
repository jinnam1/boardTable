...
class PaginationState {
  constructor(totalRecords = 0, recordsPerPage = 10, pagesPerPage = 10) {
    ...
    this.currentPage = 1; //현재 페이지 : 현제 페이지의 스타일을 달리 주기위해 필요
...
class PaginationUI {
  ...
  handlePrevClick() {
    if (!this.state.isFirstGroup) {
      ...
      this.state.currentPage =
        this.state.currentPageGroupStart + this.state.pagesPerPage - 1;
  ...
  handleLastClick() {
    const lastGroupStart =
      ...
      (this.state.totalPages % this.state.pagesPerPage) +
      1;
